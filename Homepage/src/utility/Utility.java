...
public class Utility {
	 ...
	 * @return 페이징 생성 문자열
	 */
	public static String paging(int totalRecord, int nPage, int recordPerPage, String url, int bbsno, int nowPage,
		...
		if (nowGrp >= 2) {
			 str.append("<span class='btn btn-default'><A href='./list.jsp?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>Prev</A></span>"); 
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (nPage == i) {
				 str.append("<span class='btn btn-success'>"+i+"</span>"); 
				//str.append("<span class='span_box_2'>" + i + "</span>");
			} else {
				str.append("<span class='btn btn-primary'><A href='./list.jsp?col="+col+"&word="+word+"&nowPage="+i+"'>"+i+"</A></span>");   
		...
		if (nowGrp < totalGrp) {
			 str.append("<span class='btn btn-default'><A href='./list.jsp?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>Next</A></span>"); 
	...
	/** 
	  ...
	  public MemberDTO read(String id) { 
	    ...
	    try { 
	      ...
	      sql.append(" SELECT id, password, memebername, phone, email, zipcode, address1, address2, job, joindate, picturename"); 
...
/**
    ...
    public static List<String> getDay(){
        ...
        Calendar cal = Calendar.getInstance(); //추상클래스라 new 안함.. - 객체가 아니신것
        for(int j = 0; j < 3; j++){
            list.add(sd.format(cal.getTime())); //리쓰트에는 오늘 + 어제 +_ 그제 날짜 추가됨
            cal.add(Calendar.DATE, -1); //첫번째 돌면 어제날짜! -ㅅ- 두번째는 그제! 세번째는 3일전! :>
        }
