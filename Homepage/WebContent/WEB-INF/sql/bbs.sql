...
'왕눈이', '제목', '내용', '000', sysdate, ((SELECT NVL(MAX(grpno), 0)+1 as grpno From bbs));                 
...
DELETE FROM bbs; 
 ...
 -----  -----            ----- ------ ------ 
	...
	10		부모글431		  4      2      2
     ...
     --bbsno: 항상 최대값을 찾음
     --grpno: 부모글이 추가될 때 최대값을 찾음
     --indent & ansnum은 부모글(default) - 0임
...
grpno,indent,ansnum)  
VALUES((SELECT NVL(MAX(bbsno), 0) + 1 as bbsno FROM bbs), 
'아로미', '부모글1 답변글2', '내용', '10101', sysdate,2,1,1);
